  ...
  <div class="voice-page" :class="`voice-state-${state}`">
    ...
    <Transition name="slide-panel">
      <div v-if="showSessionPanel" class="session-panel">
        ...
        <div class="session-list">
          ...
          >
            <button class="session-item-main" @click="switchSession(s.id)">
              <span class="session-item-title">{{ s.title }}</span>
              <span class="session-item-time">{{ formatTime(s.updated_at) }}</span>
            </button>
            <!-- 历史记录回看（weave-talk 解耦新增：chatbot 语音页无独立历史视图） -->
            <button class="session-history-btn" @click="openHistory(s)" title="查看对话记录" aria-label="查看对话记录">
    ...
    <Transition name="slide-panel">
      <div v-if="showHistoryPanel" class="history-panel">
        <div class="session-panel-header">
          <button class="voice-icon-btn" @click="closeHistory" title="返回" aria-label="返回">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/>
            </svg>
          </button>
          <span class="session-panel-title">{{ historyTitle }}</span>
          <button class="voice-icon-btn" @click="continueSession" title="继续此对话" aria-label="继续此对话">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="history-message-list">
          <div v-if="historyLoading" class="session-empty">加载中…</div>
          <template v-else>
            <div
              v-for="(m, i) in historyMessages"
              :key="i"
              class="history-msg"
              :class="`role-${m.role}`"
            >
              <div class="history-bubble">{{ m.content }}</div>
            </div>
            <div v-if="!historyMessages.length" class="session-empty">暂无对话内容</div>
          </template>
        </div>
      </div>
    </Transition>
...
import { getVoiceSessionMessages, type VoiceSession } from '@/api/voice'
// weave-talk 无 assistant/chat store（语音域裁剪）：退出语音页时直接断开
// 连接并返回，不再做助手/会话侧栏联动。
...
const historyMessages = ref<{ role: string; content: string }[]>([])
const historySessionId = ref<string | null>(null)
const historyTitle = ref('')

async function openHistory(s: VoiceSession) {
  historySessionId.value = s.id
  historyTitle.value = s.title || '语音会话'
  showHistoryPanel.value = true
  historyLoading.value = true
  historyMessages.value = []
  try {
    historyMessages.value = await getVoiceSessionMessages(s.id)
  } catch (e) {
...
async function continueSession() {
  const sid = historySessionId.value
  closeHistory()
  if (sid) {
    await voice.switchSession(sid)
  }
}
...
.session-history-btn:hover {
  color: var(--color-primary, #6c8eef);
...
.role-user .history-bubble {
  background: color-mix(in srgb, var(--color-primary, #6c8eef) 35%, transparent);
  border-bottom-right-radius: 4px;
}
.role-assistant .history-bubble {
  background: var(--color-hover, rgba(255,255,255,0.08));
