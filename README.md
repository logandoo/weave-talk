...
**实时会话状态机是最难的部分，也是做得最扎实的部分**：`services/voice_service.py` 约 4000 行 /
107 个方法，承载完整的双工会话编排——打断归一化比对、EoT 语义检查 + 看门狗、情绪衰减、
碎片合并、噪音门控、插话、TTS 流控与静音检测。这类"音频时序 + 事件协议 + LLM 异步"交织的
状态机极易出死锁/竞态，代码以事件队列 + 显式状态字段组织，在同类项目里属
高完成度。

**配置驱动设计优秀**：`config_model.toml` 以 `[api]/[asr]/[voice]/[providers]` 整段覆盖合并，
50+ 键全部有默认值；ASR 三模式、TTS、LLM 路由、system prompt 全部声明式配置，
不写死任何供应商。

**服务边界清晰**：ASR / TTS / LLM / provider_router / title_generator 各自独立文件，
与语音编排解耦——替换任何供应商只动配置，不动编排。

**待改进点**：
- `voice_service.py` 单文件近 4000 行，编排 + 音频细节 + 事件协议混在一起，可读性与可维护性
  是最大单点维护负担；建议按"事件协议 / 会话状态机 / 音频流处理"拆模块
- 意图分类子代理对过短/无语义输入（如单独 "smoke"）会判定为噪声并下发 `user_turn_cancelled`，
  属原生语义，但新用户易误判为故障（README 已注明）
- 配置键 40+ 且多数无注释，新手调参成本高，建议后续为每个键补一行说明

**总体评价**：实时语音交互的工程复杂度与完成度都很高；核心资产是
voice_service 的双工状态机与全套降级路径。主要债务是单文件过大与配置文档密度不足。
