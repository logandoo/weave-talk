...
class TitleGeneratorService:
    ...
    async def _request_title_json(self, messages: list[dict]) -> Optional[dict]:
        ...
        while attempt < attempts:
            ...
            current_max_tokens = max_tokens if attempt == 1 else max(repair_max_tokens, max_tokens)
    ...
    async def get_fallback_title(self, user_query: str = "") -> str:
        ...
        # weave-talk: agentic_judge 不在本项目的 voice 栈依赖内（解耦点），
        # 直接走结构截断兜底——行为与原版"LLM 兜底不可用时"完全一致。
