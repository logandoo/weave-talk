...
class VoiceDuplexSession:
    ...
    def _system_prompt(self) -> str:
        # weave-talk 解耦点：system prompt 完全由 config.toml [voice].system_prompt 配置。
        return self.config.voice_system_prompt or "你是一个全双工语音对话助手，用自然口语化的中文简洁回答。"

    async def _load_identity(self) -> None:
        """Load the user's voice assistant identity.

        weave-talk 解耦点：system prompt 完全由 config.toml [voice].system_prompt
        配置（见 _system_prompt），chatbot 原版在此组装人设/记忆/技能/工具指引
        段落全部移除；仅保留 ensure_voice_assistant 副作用（_ensure_conversation
        需要 _voice_assistant 归属语音会话）。
        """
        try:
    ...
    async def _load_history(self) -> None:
        ...
        weave-talk 解耦点：仅加载 user/assistant 纯文本轮次（本项目无工具调用）。
        """
        if not self.conversation_id:
            return
    ...
    async def _classify_interjection(
        ...
        identity = (self.config.voice.get("identity") or "语音助手").strip() or "语音助手"
        system = (
            f"你是全双工语音助手\u201c{identity}\u201d。用户正在说话，每说完一句话（句号结尾），"
    ...
    def _build_interjection_context_note(self) -> str:
        ...
        for i, inj in enumerate(self._interjection_snapshot, 1):
            lines.append(f"  {i}. 用户说「{inj['sentence'][:30]}」时你说了「{strip_voice_tags(inj['text'])}」")
    ...
    async def _tts_consumer(self) -> None:
        ...
        while not self._closed:
            ...
            if self._current_seg_audio_sec > 0 and not truncated:
    ...
    async def _speak_text(self, text: str, wait: bool = True) -> None:
        ...
        for seg in segments:
            if seg.strip():
                await self._tts_queue.put({"text": seg, "epoch": self._turn_epoch})
        if wait:
    ...
    async def _on_user_turn_locked(self, text: str) -> None:
        ...
        if self._turn_active and not self._closed:
            # The user spoke while a turn is still running. Their utterance is
            # queued — acknowledge IMMEDIATELY so they are never met with
            # total silence; the queued turn is answered as soon as the
    ...
    async def _handle_user_turn(self, text: str) -> None:
        ...
        # Wait for the (already-running) generation to finish.
        try:
            await gen_task
        except asyncio.CancelledError:
            pass
    ...
    async def _generate_and_speak(self, text: str, prefetched: str | None = None) -> None:
        ...
        while True:
            ...
            # NOTE: _interrupt (TTS barge-in) does NOT abort the generation —
            # the generation continues even if the user interrupts the speech.
            # Only an explicit stop command (_task_cancelled) or disconnect
            # stops it.
                            ...
                            if etype == "content":
                                ...
                                if not _first_content:
                                    ...
                                    _perf("llm_ttft", (_now() - _llm_start) * 1000,
                                          turn_ms=(_now() - self._turn_started) * 1000)
                                reply_text += data
                                seg_buffer += data
        ...
        if full_reply.strip():
            ...
            # Persist the plain-text assistant reply (weave-talk: 无工具调用).
    ...
    async def _generate_and_speak_cancellable(self, text: str, prefetched: str | None = None) -> None:
        ...
        try:
            await self._generate_and_speak(text, prefetched=prefetched)
        except asyncio.CancelledError:
            # Noise gate cancelled this generation — roll back history and
            # notify the client so any partial display text is cleared.
        ...
        while not self._closed:
            ...
            if etype == "ready":
                ...
                if (
                    ...
                    and self._speaking_epoch == self._turn_epoch
